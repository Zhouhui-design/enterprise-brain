...
async function testCascadeDelete() {
  try {
    ...
    // 2. 检查工序计划表结构
    console.log('\n2. 检查工序计划表结构:');
    const [processColumns] = await pool.execute('SHOW COLUMNS FROM process_plans WHERE Field LIKE "%order%"');
    ...
    // 4. 查询现有工序计划数据
    console.log('\n4. 查询现有工序计划数据:');
    const [processPlans] = await pool.execute('SELECT id, plan_no, sales_order_no FROM process_plans LIMIT 5');
    ...
    const [processCount] = await pool.execute('SELECT COUNT(*) as count FROM process_plans WHERE sales_order_no = ?', [
      testOrderNo,
    ]);
