...
class SalesService {
  ...
  async getSalesMetrics(params) {
    ...
    try {
      ...
      // 计算转化率
      const conversionRate = orderMetrics.totalOrders > 0 
        ? (orderMetrics.totalOrders / quotationMetrics.totalQuotations * 100).toFixed(2)
        : 0

      const compareConversionRate = compareOrderMetrics.totalOrders > 0 
      ...
      return {
        ...
        customers: {
          ...
          trendPercentage: this.calculateTrendPercentage(customerMetrics.totalCustomers, compareCustomerMetrics.totalCustomers)
        },
        conversion: {
        ...
        averageCustomerValue: customerMetrics.totalCustomers > 0 ? orderMetrics.totalRevenue / customerMetrics.totalCustomers : 0
      }
    } catch (error) {
    ...
    try {
      ...
      orderData.forEach(order => {
        order.items.forEach(item => {
          ...
          // 计算利润率（假设有成本价）
          const profitMargin = item.product.costPrice > 0 
            ? ((item.unitPrice - item.product.costPrice) / item.unitPrice * 100)
            : 0
    ...
    try {
      if (type === 'type') {
        ...
        const regionData = await Customer.findAll({
          attributes: [
            'regionId',
            [sequelize.fn('COUNT_DISTINCT', sequelize.col('id')), 'count']
          ],
          include: [{
  ...
  async addProductTrends(products, dateCondition) {
    const compareDateCondition = this.getComparePeriod('month', dateCondition.createdAt?.[Op.gte], dateCondition.createdAt?.[Op.lte])
