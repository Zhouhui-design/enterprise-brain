...
class SprayPaintingProcessPlanService {
    ...
    try {
      ...
      const dataSQL = `
        SELECT 
          ...
          (
            SELECT COUNT(*)
            FROM spray_painting_process_plans AS rpp2
            WHERE rpp2.process_name = spray_painting_process_plans.process_name
              AND DATE_FORMAT(rpp2.schedule_date, '%Y-%m-%d') = DATE_FORMAT(spray_painting_process_plans.schedule_date, '%Y-%m-%d')
              AND rpp2.id <= spray_painting_process_plans.id
          ) as daily_plan_count,
  ...
  static async checkAndCreateIncremental(sourceRecordId, maxDepth = 100, currentDepth = 0) {
    ...
    try {
      ...
      const createResult = await SprayPaintingProcessPlanService.create(incrementalData);
...
module.exports = SprayPaintingProcessPlanService;