...
class ListStyleProductionBomService {
  ...
  static async getListStyleBomById(id) {
    try {
      const [rows] = await pool.execute('SELECT * FROM list_style_production_boms WHERE id = ?', [id]);
  ...
  static async generateFromProductionBom(productionBomId, mode = 'check') {
    ...
    try {
      ...
      // 1. 获取生产BOM信息
      const [productionBomRows] = await connection.execute('SELECT * FROM production_boms WHERE id = ?', [
        productionBomId,
      ]);
      ...
      parentItems.push(
        ...childParentComponents.map(comp => ({
      ...
      for (const parentItem of parentItems) {
        ...
        if (conflict && mode === 'replace') {
          ...
          await connection.execute('DELETE FROM list_style_production_boms WHERE parent_code = ?', [
            parentItem.component_code,
          ]);
          replacedCount++;
        ...
        console.log(
          `  └─ 父件: ${parentItem.component_code} (层阶: ${parentLevel}, 产出工序: ${parentItem.output_process})`,
        );
      ...
      console.log(
        `✅ 成功从生产BOM ${productionBomId} 生成 ${createdCount} 条列表式BOM记录（跳过${skippedCount}条重复，覆盖${replacedCount}条）`,
      );
  ...
  static async updateListStyleBom(id, bomData) {
    ...
    try {
      ...
      // 删除原有子件
      await connection.execute('DELETE FROM list_style_bom_children WHERE parent_id = ?', [id]);
  ...
  static async deleteListStyleBom(id) {
    try {
      ...
      try {
        ...
        // 删除主记录
        const [result] = await connection.execute('DELETE FROM list_style_production_boms WHERE id = ?', [id]);
  ...
  static async batchDeleteListStyleBom(ids) {
    try {
      ...
      try {
        ...
        // 删除子件
        await connection.execute(`DELETE FROM list_style_bom_children WHERE parent_id IN (${placeholders})`, ids);
  ...
  static async generateSequence() {
    try {
      const [rows] = await pool.execute('SELECT MAX(sequence) as maxSequence FROM list_style_production_boms');
