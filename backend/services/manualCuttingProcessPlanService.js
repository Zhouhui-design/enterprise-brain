 ...
 * 人工下料工序计划服务
 */
class ManualCuttingProcessPlanService {
  /**
   * 获取所有人工下料工序计划(分页)
   */
  static async getAll(params = {}) {
    try {
      ...
      // 查询总数
      const countSQL = `SELECT COUNT(*) as total FROM manual_cutting_process_plans ${whereSQL}`;
      ...
      const dataSQL = `
        SELECT 
          ...
          (
            SELECT COUNT(*)
            FROM manual_cutting_process_plans AS rpp2
            WHERE rpp2.process_name = manual_cutting_process_plans.process_name
              AND DATE_FORMAT(rpp2.schedule_date, '%Y-%m-%d') = DATE_FORMAT(manual_cutting_process_plans.schedule_date, '%Y-%m-%d')
              AND rpp2.id <= manual_cutting_process_plans.id
          ) as daily_plan_count,
          created_at, updated_at
        FROM manual_cutting_process_plans 
        ${whereSQL}
    ...
    try {
      ...
      const [rows] = await pool.execute(
        ...
        SELECT 
          ...
          created_at, updated_at
        FROM manual_cutting_process_plans WHERE id = ?
      `,
        [id],
      );
      ...
      console.error('获取人工下料工序计划详情失败:', error);
      throw error;
    }
  }
  ...
  static async create(data) {
    try {
      ...
      // 正确的SQL，包含所有字段，数量匹配
      const sql = `
        INSERT INTO manual_cutting_process_plans (
      ...
      console.log(`人工下料工序计划创建成功, ID: ${result.insertId}, 编号: ${data.planNo}`);

      // ✅ 修改：自动推送到备料计划
      // 触发时机：不管什么原因新增的人工下料工序计划行，都要检查推送条件
      // 推送条件：计划排程数量 > 0
      console.log(`\n🔍 [自动推送检查] 人工下料工序计划 -> 备料计划`);
      console.log(`   人工下料工序计划ID: ${result.insertId}`);
      console.log(`   人工下料工序计划编号: ${data.planNo}`);
      ...
      if (data.scheduleQuantity && parseFloat(data.scheduleQuantity) > 0) {
        try {
          ...
          console.log(`   查询到 ${createdPlanRows.length} 条人工下料工序计划记录`);
          ...
          if (createdPlanRows.length > 0) {
            ...
            // ✅ 注：备料计划推送到人工下料工序计划的触发已移动到 realProcessPlanToMaterialService.pushToMaterialPreparation 的commit后
          } else {
            console.warn(`   ⚠️ 未查询到刚创建的人工下料工序计划记录`);
          }
        } catch (error) {
          console.error(`\n❌ 自动推送到备料计划失败:`);
  ...
  static async update(id, data) {
    try {
      const sql = `
        UPDATE manual_cutting_process_plans SET
      ...
      if (result.affectedRows === 0) {
        throw new Error('人工下料工序计划不存在或未更新');
      }

      console.log(`人工下料工序计划更新成功, ID: ${id}`);
      return { id };
    } catch (error) {
  ...
  static async delete(id) {
    ...
    try {
      ...
      // ✅ 步颊1: 先查询人工下料工序计划详情(用于后续释放已占用工时)
      const [planRows] = await connection.execute(
        "SELECT plan_no, process_name, DATE_FORMAT(schedule_date, '%Y-%m-%d') as schedule_date FROM manual_cutting_process_plans WHERE id = ?",
        [id],
      );

      if (planRows.length === 0) {
        await connection.rollback();
        throw new Error('人工下料工序计划不存在');
      }

      const plan = planRows[0];
      console.log(`🗑️ 删除人工下料工序计划: ${plan.plan_no}`);

      // ✅ 步颊2: 执行删除
      const [result] = await connection.execute('DELETE FROM manual_cutting_process_plans WHERE id = ?', [id]);
      ...
      console.log(`✅ 人工下料工序计划删除成功, ID: ${id}`);
      ...
      if (plan.process_name && plan.schedule_date) {
        try {
          ...
          // ✅ SUMIF - 重新统计该工序+日期下所有人工下料工序计划的计划排程工时总和
  ...
  static async batchDelete(ids) {
    ...
    try {
      ...
      await connection.commit();
      console.log(`批量删除人工下料工序计划完成: 成功${successCount}条/总共${ids.length}条`);
  ...
  static async checkAndCreateIncremental(sourceRecordId, maxDepth = 100, currentDepth = 0) {
    ...
    try {
      ...
      const [records] = await connection.execute(
        `
        SELECT * FROM manual_cutting_process_plans WHERE id = ?
      `,
        [sourceRecordId],
      );
      ...
      const createResult = await ManualCuttingProcessPlanService.create(incrementalData);
      ...
      if (sourceRecord.source_no) {
        ...
        await connection.execute(
          `
          UPDATE manual_cutting_process_plans 
      ...
      if (newUnscheduledQty > 0 && newNextScheduleDate) {
        ...
        await ManualCuttingProcessPlanService.checkAndCreateIncremental(newRecordId, maxDepth, currentDepth + 1);
      } else {
...
module.exports = ManualCuttingProcessPlanService;
