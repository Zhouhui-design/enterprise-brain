 ...
 * 激光切管工序计划服务
 */
class LaserTubeCuttingProcessPlanService {
  /**
   * 获取所有激光切管工序计划(分页)
   */
  static async getAll(params = {}) {
    try {
      ...
      // 查询总数
      const countSQL = `SELECT COUNT(*) as total FROM laser_tube_cutting_process_plans ${whereSQL}`;
      ...
      const dataSQL = `
        SELECT 
          ...
          (
            SELECT COUNT(*)
            FROM laser_tube_cutting_process_plans AS rpp2
            WHERE rpp2.process_name = laser_tube_cutting_process_plans.process_name
              AND DATE_FORMAT(rpp2.schedule_date, '%Y-%m-%d') = DATE_FORMAT(laser_tube_cutting_process_plans.schedule_date, '%Y-%m-%d')
              AND rpp2.id <= laser_tube_cutting_process_plans.id
          ) as daily_plan_count,
          created_at, updated_at
        FROM laser_tube_cutting_process_plans 
        ${whereSQL}
    ...
    try {
      ...
      const [rows] = await pool.execute(`
        SELECT 
          ...
          created_at, updated_at
        FROM laser_tube_cutting_process_plans WHERE id = ?
      `, [id]);
      ...
      console.error('获取激光切管工序计划详情失败:', error);
      throw error;
    }
  }
  ...
  static async create(data) {
    try {
      ...
      console.log(`激光切管工序计划创建成功, ID: ${result.insertId}, 编号: ${data.planNo}`);
      
      // ✅ 修改：自动推送到备料计划
      // 触发时机：不管什么原因新增的激光切管工序计划行，都要检查推送条件
      // 推送条件：计划排程数量 > 0
      console.log(`\n🔍 [自动推送检查] 激光切管工序计划 -> 备料计划`);
      console.log(`   激光切管工序计划ID: ${result.insertId}`);
      console.log(`   激光切管工序计划编号: ${data.planNo}`);
      ...
      if (data.scheduleQuantity && parseFloat(data.scheduleQuantity) > 0) {
        try {
          ...
          console.log(`   查询到 ${createdPlanRows.length} 条激光切管工序计划记录`);
          ...
          if (createdPlanRows.length > 0) {
            ...
            // ✅ 注：备料计划推送到激光切管工序计划的触发已移动到 realProcessPlanToMaterialService.pushToMaterialPreparation 的commit后
          } else {
            console.warn(`   ⚠️ 未查询到刚创建的激光切管工序计划记录`);
          }
        } catch (error) {
          console.error(`\n❌ 自动推送到备料计划失败:`);
  ...
  static async update(id, data) {
    try {
      const sql = `
        UPDATE laser_tube_cutting_process_plans SET
      ...
      if (result.affectedRows === 0) {
        throw new Error('激光切管工序计划不存在或未更新');
      }
      
      console.log(`激光切管工序计划更新成功, ID: ${id}`);
      return { id };
    } catch (error) {
  ...
  static async delete(id) {
    ...
    try {
      ...
      // ✅ 步颊1: 先查询激光切管工序计划详情(用于后续释放已占用工时)
      const [planRows] = await connection.execute(
        'SELECT plan_no, process_name, DATE_FORMAT(schedule_date, \'%Y-%m-%d\') as schedule_date FROM laser_tube_cutting_process_plans WHERE id = ?',
        [id]
      );
      
      if (planRows.length === 0) {
        await connection.rollback();
        throw new Error('激光切管工序计划不存在');
      }
      
      const plan = planRows[0];
      console.log(`🗑️ 删除激光切管工序计划: ${plan.plan_no}`);
      
      // ✅ 步颊2: 执行删除
      const [result] = await connection.execute('DELETE FROM laser_tube_cutting_process_plans WHERE id = ?', [id]);
      ...
      console.log(`✅ 激光切管工序计划删除成功, ID: ${id}`);
      ...
      if (plan.process_name && plan.schedule_date) {
        try {
          ...
          // ✅ SUMIF - 重新统计该工序+日期下所有激光切管工序计划的计划排程工时总和
  ...
  static async batchDelete(ids) {
    ...
    try {
      ...
      await connection.commit();
      console.log(`批量删除激光切管工序计划完成: 成功${successCount}条/总共${ids.length}条`);
  ...
  static async checkAndCreateIncremental(sourceRecordId, maxDepth = 100, currentDepth = 0) {
    ...
    try {
      ...
      const [records] = await connection.execute(`
        SELECT * FROM laser_tube_cutting_process_plans WHERE id = ?
      `, [sourceRecordId]);
      ...
      const createResult = await LaserTubeCuttingProcessPlanService.create(incrementalData);
      ...
      if (sourceRecord.source_no) {
        ...
        // 19. 更新刚创建的记录
        await connection.execute(`
          UPDATE laser_tube_cutting_process_plans 
      ...
      if (newUnscheduledQty > 0 && newNextScheduleDate) {
        ...
        await LaserTubeCuttingProcessPlanService.checkAndCreateIncremental(newRecordId, maxDepth, currentDepth + 1);
      } else {
...
module.exports = LaserTubeCuttingProcessPlanService;