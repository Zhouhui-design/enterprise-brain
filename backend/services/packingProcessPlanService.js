      ...
      if (data.scheduleDate && data.processName) {
        try {
          ...
          if (capacityRows.length > 0) {
            ...
            console.log(
              `✅ [当天总工时查询] ${capacity.available_workstations} × ${capacity.work_shift} = ${dailyTotalHours}`,
            );
          } else {
    ...
    try {
      ...
      if (sourceRecord.source_no) {
        ...
        for (const record of allRecords) {
          ...
          await connection.execute(`UPDATE packing_process_plans SET remaining_required_hours = ? WHERE id = ?`, [
            recordRemainingHours,
            record.id,
          ]);
        }
      ...
      if (newUnscheduledQty > 0 && newNextScheduleDate) {
        ...
        await PackingProcessPlanService.checkAndCreateIncremental(
          newRecordId,
          frontEndData,
          maxDepth,
          currentDepth + 1,
