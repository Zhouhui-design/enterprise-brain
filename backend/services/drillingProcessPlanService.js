...
class DrillingProcessPlanService {
   ...
   * @param {object} frontEndData - 前端传递的数据
  ...
  static async checkAndCreateIncremental(sourceRecordId, frontEndData = {}, maxDepth = 100, currentDepth = 0) {
    ...
    try {
      ...
      const [records] = await connection.execute(
        `
        SELECT *, next_schedule_date1 FROM drilling_process_plans WHERE id = ?
      `,
        [sourceRecordId],
      );
      ...
      if (newUnscheduledQty > 0 && newNextScheduleDate) {
        ...
        await DrillingProcessPlanService.checkAndCreateIncremental(
          newRecordId,
          frontEndData,
          maxDepth,
          currentDepth + 1,
