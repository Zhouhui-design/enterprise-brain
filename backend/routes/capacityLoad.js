...
router.get('/list', async (req, res) => {
  try {
    const { page = 1, pageSize = 20, processName, startDate, endDate, sortBy = 'date', sortOrder = 'ASC' } = req.query;
    ...
    if (rows.length === 0) {
      ...
      if (allRows.length > 0) {
        ...
        if (qualifiedRows.length > 0) {
          console.log(
            `   最近的符合条件的日期: ${qualifiedRows[0].date} (剩余工时=${qualifiedRows[0].remaining_hours})`,
          );
        }
      } else {
...
router.post('/query-daily-total-hours', async (req, res) => {
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      console.log(
        `✅ 找到当天总工时: ${dailyTotalHours} (可用工位=${result.available_workstations}, 上班时段=${result.work_shift})`,
      );
  ...
  try {
    ...
    for (const record of capacityRecords) {
      try {
        ...
        console.log(
          `🔍 处理记录: 工序=${processName}, 原始日期=${date}, 格式化日期=${formattedDate}, 当前占用工时=${previousOccupiedHours}`,
        );
        ...
        if (hoursChanged) {
          ...
          console.log(
            `✅ [工序=${processName}, 日期=${formattedDate}] ${previousOccupiedHours} → ${newOccupiedHours} (释放${releasedHours.toFixed(2)}小时)`,
          );
        } else {
          console.log(
            `⚪ [工序=${processName}, 日期=${formattedDate}] 无需更新: ${previousOccupiedHours} = ${newOccupiedHours}`,
          );
        }
      } catch (error) {
    ...
    for (const process of processes) {
      ...
      for (let i = 0; i < displayDays; i++) {
        ...
        if (availableWorkstationsValue > 0) {
          remainingShift = parseFloat((remainingHours / availableWorkstationsValue).toFixed(2)).toString();
        }
...
router.get('/settings/:pageKey', async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute('SELECT setting_key, setting_value FROM page_settings WHERE page_key = ?', [
      pageKey,
    ]);
...
router.post('/reset-work-shift', async (req, res) => {
  ...
  try {
    ...
    // ✅ 修复：使用calendar_date（日历日期）查询企业日历数据
    // 原因：工序能力负荷表的date字段在前端显示时会+8小时（UTC转本地）
    // 因此应该匹配企业日历的calendar_date（也会+8小时后显示）
    // 规则：工序能力负荷表.date = 企业日历.calendar_date → 标准上班时长
    ...
    for (const calendar of calendarData) {
      const calendarDate = calendar.calendar_date; // 日历日期（已格式化为YYYY-MM-DD）
      const hours = calendar.standard_work_hours;
      const isWorkday = calendar.is_workday;
      const weekday = calendar.weekday;

      console.log(`  处理日期: ${calendarDate}, ${weekday}, 是否工作日: ${isWorkday}, 标准工时: ${hours}`);
      ...
      // ✅ 更新该日期所有工序的上班时段
      // 匹配规则：工序能力负荷表.date = 企业日历.calendar_date
    ...
    const [unmatchedResult] = await connection.execute(`
      ...
      LEFT JOIN company_calendar cc ON DATE_FORMAT(pcl.date, '%Y-%m-%d') = DATE_FORMAT(cc.calendar_date, '%Y-%m-%d')
      SET pcl.work_shift = NULL
      WHERE cc.calendar_date IS NULL
    `);
...
router.delete('/cleanup', async (req, res) => {
  try {
    ...
    const [result] = await pool.execute('DELETE FROM process_capacity_load WHERE date < ?', [todayStr]);
...
router.post('/reset-remaining-hours', async (req, res) => {
  ...
  try {
    ...
    for (const record of records) {
      ...
      const remainingHours = parseFloat((workShift * availableWorkstations - occupiedHours).toFixed(2));

      // 计算剩余时段 = 剩余工时 ÷ 可用工位数量
      let remainingShift = '0.00';
      if (availableWorkstations > 0) {
        remainingShift = parseFloat((remainingHours / availableWorkstations).toFixed(2)).toString();
      }
...
router.post('/query-next-schedule-date', async (req, res) => {
  ...
  try {
    ...
    console.log('🔍 查询下一个排程日期参数:', { processName, scheduleDate, minRemainingHours });

    // 验证必要参数
    if (!processName || !scheduleDate) {
      console.log('❌ 缺少必要参数:', { processName, scheduleDate });
      return res.status(400).json({
        code: 400,
        message: '缺少必要参数：processName 和 scheduleDate',
      });
    ...
    const sql =
      "SELECT DATE_FORMAT(date, '%Y-%m-%d') as formatted_date, remaining_hours FROM process_capacity_load WHERE process_name = ? AND DATE_FORMAT(date, '%Y-%m-%d') >= ? AND remaining_hours >= ? ORDER BY date ASC LIMIT 1";
