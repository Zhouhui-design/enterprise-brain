  ...
  try {
    ...
    for (const processData of processesData) {
      try {
        ...
        // 检查是否存在
        const [existing] = await connection.execute('SELECT id FROM processes WHERE process_code = ?', [processCode]);
  ...
  try {
    ...
    if (oldProcess.self_or_outsource !== newSelfOrOutsource) {
      if (oldProcess.self_or_outsource === '自制' && newSelfOrOutsource === '外协') {
        ...
        const [deleteResult] = await connection.execute('DELETE FROM process_capacity_load WHERE process_name = ?', [
          oldProcess.process_name,
        ]);
        capacityUpdateMsg += `删除能力负荷表记录${deleteResult.affectedRows}条；`;
        console.log(
          `✅ 自制→外协：删除能力负荷表中工序"${oldProcess.process_name}"的${deleteResult.affectedRows}条记录`,
        );
    ...
    if (newSelfOrOutsource === '自制' && oldProcess.process_name !== newProcessName) {
      ...
      if (updateNameResult.affectedRows > 0) {
        ...
        console.log(
          `✅ 工序名称变化：能力负荷表中"${oldProcess.process_name}"→"${newProcessName}"，更新${updateNameResult.affectedRows}条`,
        );
      }
    }
    ...
    if (newSelfOrOutsource === '自制' && oldProcess.available_workstations !== newAvailableWorkstations) {
      ...
      if (updateWorkstationsResult.affectedRows > 0) {
        ...
        console.log(
          `✅ 可用工位数量变化：能力负荷表中"${targetProcessName}"的可用工位数量→${newAvailableWorkstations}，更新${updateWorkstationsResult.affectedRows}条`,
        );
  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (processData.length > 0) {
        ...
        if (deleteCapacityResult.affectedRows > 0) {
          console.log(
            `✅ 同步删除能力负荷表中工序"${process.process_name}"的${deleteCapacityResult.affectedRows}条记录`,
          );
        }
      }
