...
router.post('/', (req, res) => {
  try {
    const {
      orderNumber, customerName, customerContact, customerPhone,
      shipToAddress, planShipDate, actualShipDate, totalAmount,
      status = 'DRAFT', remark, items = []
    } = req.body
    ...
    stmt.run(
      id, planNumber, orderNumber, customerName, customerContact,
      customerPhone, shipToAddress, planShipDate, actualShipDate,
      totalAmount || 0, status, 'admin', remark
    )
    ...
    if (items && items.length > 0) {
      ...
      for (const item of items) {
        itemStmt.run(
          id, item.productCode, item.productName, item.quantity || 0,
          item.unit, item.unitPrice || 0, item.totalPrice || 0
        )
      }
    }
