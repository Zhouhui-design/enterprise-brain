...
router.post('/batch-query', async (req, res) => {
  try {
    ...
    // ✅ 修复：使用calendar_date匹配工序能力负荷表的date
    // 因为两者在前端显示时都会经过UTC→本地时区转换（+8小时）
    // 构建 IN 查询
...
router.get('/settings/:pageKey', async (req, res) => {
  try {
    ...
    const [rows] = await pool.query('SELECT setting_key, setting_value FROM page_settings WHERE page_key = ?', [
      pageKey,
    ]);
...
router.get('/custom-holidays', async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT * FROM custom_holidays ORDER BY month, day');
...
async function dailyUpdateCalendar() {
  ...
  try {
    ...
    const [deleteResult] = await connection.execute('DELETE FROM company_calendar WHERE calendar_date < ?', [
      cutoffDateStr,
    ]);
    ...
    try {
      ...
      for (const calendar of calendarData) {
        ...
        if (dateStr.startsWith('2025-12-1') || dateStr.startsWith('2025-12-2')) {
          console.log(
            `[定时任务] 同步 ${dateStr}: 标准工时=${hours}, 上班时段=${workShift}, 更新记录数=${updateResult.affectedRows}`,
          );
        }
      }
