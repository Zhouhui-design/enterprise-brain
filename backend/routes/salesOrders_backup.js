  ...
  try {
    ...
    const ordersWithProducts = await Promise.all(
      orders.map(async order => {
        const [products] = await connection.execute('SELECT * FROM sales_order_products WHERE order_id = ?', [
          order.id,
        ]);
    ...
    const formattedOrders = ordersWithProducts.map(order => {
      const customerDeliveryFormatted = order.customer_delivery
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(req.body).length === 1 && req.body.status) {
      await connection.execute('UPDATE sales_orders SET status = ?, updated_by = ? WHERE id = ?', [
        req.body.status,
        'admin',
        id,
      ]);
        ...
        for (const product of products) {
          ...
          `,
            [
              ...
              product.taxRate,
              product.totalPriceExcludingTax,
              product.totalTax,
              product.totalPrice,
              product.accessories ? JSON.stringify(product.accessories) : null,
              product.outputProcess || null, // ✅ 关键：保存产出工序
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    try {
      const [processPlanResult] = await connection.execute('DELETE FROM real_process_plans WHERE sales_order_no = ?', [
        internalOrderNo,
      ]);
    ...
    for (const tableName of otherProcessTables) {
      try {
        const [result] = await connection.execute(`DELETE FROM ${tableName} WHERE sales_order_no = ?`, [
          internalOrderNo,
        ]);
    ...
    // 🗂️ 阶段7：删除订单产品和支付计划
    try {
      const [productResult] = await connection.execute('DELETE FROM order_products WHERE order_id = ?', [id]);
      console.log(`✅ 级联删除订单产品: ${productResult.affectedRows} 条`);

      const [paymentResult] = await connection.execute('DELETE FROM order_payment_schedules WHERE order_id = ?', [id]);
    ...
    // 🗂️ 阶段8：删除主订单记录
    try {
      const [orderResult] = await connection.execute('DELETE FROM sales_orders WHERE id = ?', [id]);
...
router.post('/batch-delete', async (req, res) => {
  ...
  try {
    ...
    for (const id of ids) {
      ...
      const [orderRows] = await connection.execute('SELECT internal_order_no FROM sales_orders WHERE id = ?', [id]);
  ...
  try {
    ...
    try {
      ...
      for (const orderId of ids) {
        // 1. 查询订单及产品明细
        const [orders] = await connection.execute('SELECT * FROM sales_orders WHERE id = ?', [orderId]);
        ...
        const [products] = await connection.execute('SELECT * FROM sales_order_products WHERE order_id = ?', [orderId]);
        ...
        for (const product of products) {
          ...
          let productImage = product.product_image || null;
          ...
          if (productCode) {
            try {
              ...
              if (productRows.length > 0) {
                ...
                console.log('⚠️ 未找到产品:', productCode);
              }
            } catch (lookupError) {
          ...
          if (outputProcess === '采购' && suggestedQty > 0) {
            ...
            await connection.execute(
              `INSERT INTO master_production_plans (
                ...
                product_image, product_source, submitter
              ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                planCode,
                productCode,
        ...
        // 5. 更新订单状态
        await connection.execute('UPDATE sales_orders SET status = ? WHERE id = ?', ['已确认', orderId]);
      }
