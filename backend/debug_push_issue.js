...
async function diagnose() {
  try {
    ...
    realPlans.forEach((plan, index) => {
      ...
      console.log(
        `   是否满足推送条件 (排程数量>0): ${plan.schedule_quantity && parseFloat(plan.schedule_quantity) > 0 ? '✅ 满足' : '❌ 不满足'}`,
      );
    });
    ...
    materialPlans.forEach((plan, index) => {
      ...
      // 检查6个新增字段是否完整
      const hasNewFields =
        plan.customer_order_no &&
        plan.main_plan_product_code &&
        plan.main_plan_product_name &&
        plan.promise_delivery_date;
