  ...
  try {
    ...
    if (realPlans.length > 0) {
      ...
      if (realPlan.schedule_date) {
        const dateObj = realPlan.schedule_date instanceof Date 
          ? realPlan.schedule_date 
          : new Date(realPlan.schedule_date);
      ...
      if (materialPlans.length > 0) {
        ...
        materialPlans.forEach((plan, index) => {
          ...
          if (plan.process_schedule_date) {
            const dateObj = plan.process_schedule_date instanceof Date 
