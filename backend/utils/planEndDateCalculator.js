...
class PlanEndDateCalculator {
  ...
  static async calculate(params) {
    try {
      const { replenishmentQty, standardWorkQuota, planStartDate, scheduleDate, processName } = params;
      ...
      while (accumulatedHours < requiredWorkHours && daysChecked < maxDays) {
        ...
        if (capacityRows.length > 0) {
          ...
          if (dailyAvailableHours > 0) {
            ...
            console.log(
              `   ${currentDate.toISOString().split('T')[0]}: +${dailyAvailableHours}h (累计: ${accumulatedHours.toFixed(2)}h / ${requiredWorkHours}h)`,
            );
          } else {
      ...
      console.log(
        `   需求工时: ${requiredWorkHours}h, 累计工时: ${accumulatedHours.toFixed(2)}h, 耗时: ${daysChecked}天`,
      );
