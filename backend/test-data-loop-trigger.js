...
async function testDataLoopTrigger() {
  ...
  try {
    ...
    const expectedLoopPlans = materialPlans.filter(p => 
      p.material_source === '自制' && parseFloat(p.replenishment_quantity) > 0
    ).length;
    ...
    console.log('\n🧹 是否清理测试数据？(此测试脚本不自动清理)');
    console.log('   手动清理SQL：');
    console.log(`   DELETE FROM real_process_plans WHERE plan_no = '${realProcessPlanData.planNo}' OR source_no LIKE 'TEST-LOOP-MPP-%';`);
    console.log(`   DELETE FROM material_preparation_plans WHERE source_process_plan_no = '${realProcessPlanData.planNo}';`);
