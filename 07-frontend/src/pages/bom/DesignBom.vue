<template>
  <div class="design-bom-container">
    <!-- 顶部工具栏 -->
    <div class="toolbar">
    ...
    <!-- 导入对话框 -->
    <el-dialog v-model="importDialogVisible" title="导入设计BOM" width="500px">
      <el-upload
        class="upload-demo"
        drag
        action="#"
...
const dialogTitle = computed(() => isEdit.value ? '编辑设计BOM' : '新增设计BOM')
...
const handleCreate = () => {
  isEdit.value = false
  formData.value = {
    bomCode: `DBOM-${new Date().getFullYear()}-${String(nextBomId.value).padStart(3, '0')}`,
    bomName: '',
    productCode: '',
    productName: '',
    version: 'V1.0',
...
const handleDelete = async (row) => {
  try {
    ...
    if (index !== -1) {
      ...
      // 从数据库删除
      await databaseService.deleteBom(row.id, 'designBoms')
      updateStats()
...
const handleBatchDelete = async () => {
  try {
    ...
    for (const id of deleteIds) {
      await databaseService.deleteBom(id, 'designBoms')
    }
    updateStats()
  ...
  try {
    if (isEdit.value) {
      ...
      if (index !== -1) {
        ...
        // 保存到数据库
        await databaseService.saveBom(tableData.value[index], 'designBoms')
      }
      ...
      nextBomId.value++
      // 保存到数据库
      await databaseService.saveBom(newBom, 'designBoms')
...
const handleExport = () => {
  ...
  link.download = `设计BOM_${new Date().getTime()}.csv`
  link.click()
...
onMounted(async () => {
  try {
    ...
    // 从数据库加载BOM数据
    const boms = await databaseService.getAllBoms('designBoms')
    if (Array.isArray(boms) && boms.length > 0) {
      tableData.value = boms
    }
    
    // 获取下一个BOM ID
    nextBomId.value = await databaseService.getNextBomId('designBoms')
  } catch (error) {
    ...
    // 降级到localStorage
    // 从localStorage加载数据
    const storedData = localStorage.getItem('designBomData')
    if (storedData) {
      try {
        ...
        console.error('加载设计BOM数据失败:', e)
      }
    }
    
    // 加载下一个BOM ID
    const storedNextId = localStorage.getItem('designBomNextId')
    if (storedNextId) {
