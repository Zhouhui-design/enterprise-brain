    ...
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span>报价审核 - {{ quotationDetail.quotationNo }}</span>
      ...
      <el-form :model="reviewForm" ref="reviewFormRef" label-width="100px" class="review-form">
        ...
        <el-form-item label="审核意见" :required="reviewForm.result === 'rejected'">
          <el-input
...
const handleSubmitReview = async () => {
  ...
  try {
    ...
    // 显示成功消息
    const message = reviewForm.result === 'approved' ? '审核通过' : '审核拒绝';
