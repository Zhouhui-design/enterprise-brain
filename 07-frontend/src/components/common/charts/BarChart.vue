<template>
  ...
  >
    <div ref="chartContainerRef" class="bar-chart-container" :style="{ width: '100%', height: '100%' }" />
  </BaseChart>
</template>
...
const mergedOptions = computed(() => {
  const baseOptions: echarts.EChartsOption = {
    tooltip: {
      trigger: 'axis',
      axisPointer: {
        type: props.isHorizontal ? 'shadow' : 'cross'
      }
    },
    legend: {
    ...
    xAxis: props.isHorizontal ? {
      ...
      axisLabel: {
        rotate: processedData.value.length > 8 ? 45 : 0
      }
    },
    ...
    series: props.yFields.map((field, index) => ({
      ...
      barWidth: props.isHorizontal ? '60%' : 'auto'
    }))
  }
