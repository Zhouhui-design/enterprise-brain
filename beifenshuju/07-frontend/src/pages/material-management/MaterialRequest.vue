  ...
  <div class="material-request-page">
    ...
    <div class="stats-cards">
      <el-card class="stat-card">
        <div class="stat-content">
          ...
          <div class="stat-label">本月申请单</div>
        </div>
        <div class="stat-icon">
    ...
    <el-card class="search-card">
      <el-form :inline="true" :model="searchForm" class="search-form">
        <el-form-item label="申请单号">
          <el-input v-model="searchForm.requestNo" placeholder="请输入申请单号" clearable />
        </el-form-item>
    ...
    <el-card class="data-card">
      <div class="table-header">
        <span class="title">申请单列表</span>
        <el-button type="danger" :disabled="selectedRequests.length === 0" @click="batchDelete">
      ...
      >
        ...
        <el-table-column prop="purpose" label="申请用途" min-width="150" />
        ...
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="scope">
            <el-button size="small" @click="viewRequest(scope.row)">
            ...
            <el-button 
              v-if="hasApprovalPermission && scope.row.status === 'pending'"
              size="small" 
              type="primary"
    ...
    >
      ...
      >
        <el-row :gutter="20">
          <el-col :span="12">
            <el-form-item label="申请部门" prop="departmentId">
              <el-select v-model="requestForm.departmentId" placeholder="请选择申请部门" filterable>
                <el-option 
                  v-for="dept in departments" 
                  :key="dept.id" 
                  :label="dept.name" 
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="12">
            <el-form-item label="申请用途" prop="purpose">
              <el-select v-model="requestForm.purpose" placeholder="请选择申请用途">
                <el-option label="生产使用" value="production" />
                <el-option label="研发测试" value="research" />
        ...
        <el-form-item label="期望到货日期" prop="expectedArrivalDate">
          <el-date-picker
            v-model="requestForm.expectedArrivalDate"
            type="date"
        ...
        <el-form-item>
          ...
          <el-table
            v-if="requestForm.items.length > 0"
            :data="requestForm.items"
            style="width: 100%"
            border
          >
      ...
      <el-card v-for="(item, index) in detailSections" :key="index">
        ...
        <template v-if="item.type === 'info'">
          <el-descriptions :column="2" border>
            <el-descriptions-item label="申请单号">{{ detailInfo.requestNo }}</el-descriptions-item>
            ...
            <el-descriptions-item label="申请用途">{{ getPurposeLabel(detailInfo.purpose) }}</el-descriptions-item>
            <el-descriptions-item label="期望到货日期">{{ detailInfo.expectedArrivalDate }}</el-descriptions-item>
            ...
            <el-descriptions-item label="申请说明" :span="2">{{ detailInfo.description || '-' }}</el-descriptions-item>
    ...
    >
      <el-form :model="approveForm" label-width="100px">
        <el-form-item label="审批结果">
          <el-radio-group v-model="approveForm.approveResult" style="margin-top: 5px;">
...
import { 
  ...
  View, Edit, Delete as DeleteIcon
...
const formRules = {
  ...
  description: [
    ...
    { min: 5, message: '申请说明至少5个字符', trigger: 'blur' }
  ]
}
...
const loadRequestList = async () => {
  ...
  try {
    ...
    // 模拟数据
    const mockData = generateMockRequestData()
    
    // 模拟分页
    const start = (pagination.currentPage - 1) * pagination.pageSize
    const end = start + pagination.pageSize
    requestList.value = mockData.slice(start, end)
...
const loadStatistics = async () => {
  try {
    ...
    // 模拟统计数据
    totalRequests.value = 68
    pendingRequests.value = 18
    approvedRequests.value = 45
    rejectedRequests.value = 5
  } catch (error) {
...
const showCreateDialog = () => {
  dialogTitle.value = '新增申请单'
  resetForm()
  dialogVisible.value = true
  nextTick(() => {
    requestForm.applyDate = new Date().toISOString().split('T')[0]
    requestForm.requestNo = generateRequestNo()
  })
}

// 显示编辑对话框
const editRequest = (row) => {
  dialogTitle.value = '编辑申请单'
  resetForm()
  
  // 深拷贝选中行数据
  Object.assign(requestForm, JSON.parse(JSON.stringify(row)))
...
const batchDelete = async () => {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedRequests.value.length} 个申请单吗？`,
      '批量删除确认',
      {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
    ...
    // 从列表中移除选中项
    const idsToDelete = selectedRequests.value.map(item => item.id)
    requestList.value = requestList.value.filter(item => !idsToDelete.includes(item.id))
    pagination.total -= selectedRequests.value.length
    selectedRequests.value = []
    
    ElMessage.success(`成功删除 ${idsToDelete.length} 个申请单`)
    loadStatistics()
  } catch (error) {
    // 用户取消删除
  }
...
const submitApproval = async () => {
  try {
    ...
    // 更新列表中的状态
    const index = requestList.value.findIndex(item => item.id === approveForm.requestId)
    if (index > -1) {
      requestList.value[index].status = approveForm.approveResult
    }
    
    ElMessage.success(`申请单已${approveForm.approveResult === 'approve' ? '通过' : '拒绝'}审批`)
...
const confirmAddMaterials = () => {
  ...
  selectedMaterials.value.forEach(material => {
    const existingItem = requestForm.items.find(item => item.materialId === material.id)
    if (!existingItem) {
...
const removeMaterial = (row) => {
  const index = requestForm.items.findIndex(item => item.materialId === row.materialId)
  if (index > -1) {
...
// 生成模拟申请单数据
function generateMockRequestData() {
  const data = []
  const statuses = ['pending', 'approved', 'rejected', 'completed']
  const purposes = ['production', 'research', 'maintenance', 'other']
  ...
  for (let i = 1; i <= 80; i++) {
    ...
    const totalAmount = Math.random() * 15000 + 1000
    
    data.push({
      id: i,
      requestNo: `REQ202401${String(i).padStart(4, '0')}`,
      purpose: purpose,
      departmentName: dept,
      departmentId: departments.indexOf(dept) + 1,
      applicant: `申请人${i}`,
      applyDate: `2024-01-${String(Math.floor(Math.random() * 31) + 1).padStart(2, '0')}`,
      expectedArrivalDate: `2024-01-${String(Math.floor(Math.random() * 31) + 1).padStart(2, '0')}`,
...
function generateMockDetailData(id) {
  const statuses = ['pending', 'approved', 'rejected', 'completed']
  const status = statuses[Math.floor(Math.random() * statuses.length)]
  const purpose = ['production', 'research', 'maintenance', 'other'][Math.floor(Math.random() * 4)]
  ...
  if (status !== 'pending') {
    approvalHistory.push({
      ...
      comment: status === 'approved' ? '同意申请' : '申请用途不明确，需重新提交',
      operator: '李四'
    })
  ...
  for (let i = 1; i <= materialCount; i++) {
    ...
    items.push({
      ...
      usage: `用于${purpose === 'production' ? '生产' : purpose === 'research' ? '研发' : '其他'}项目`
    })
  }
  ...
  return {
    id: id,
    requestNo: `REQ202401${String(id).padStart(4, '0')}`,
    purpose: purpose,
