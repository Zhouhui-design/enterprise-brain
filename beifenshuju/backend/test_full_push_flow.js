...
async function testFullFlow() {
  ...
  try {
    ...
    materialPlans.forEach((plan, index) => {
      const isPushedFromRealPlan = plan.source_process_plan_no && 
                                   plan.source_process_plan_no.trim() !== '' && 
                                   plan.source_process_plan_no !== '/';
    ...
    // 7. 验证结果
    const pushedPlans = materialPlans.filter(p => 
      p.source_process_plan_no && 
      p.source_process_plan_no.trim() !== '' && 
      p.source_process_plan_no !== '/'
    );
