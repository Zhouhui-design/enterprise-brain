...
router.post('/execute-scheduling', async (req, res) => {
  try {
    ...
    const [masterPlans] = await pool.execute(
      'SELECT * FROM master_production_plans WHERE plan_code = ?',
      [planCode]
    );
    ...
    res.json({
      ...
      checkpoint: {
        ...
        expectedCount: materialPlans.filter(p => p.source_process === '打包' && p.material_source === '自制' && p.replenishment_quantity > 0).length,
...
router.post('/cleanup', async (req, res) => {
  try {
    ...
    // 删除主生产计划
    const [masterPlans] = await pool.execute('SELECT id, plan_code FROM master_production_plans WHERE product_code = ?', [productCode]);
