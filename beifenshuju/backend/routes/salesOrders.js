  ...
  try {
    ...
    for (const id of ids) {
      ...
      if (orderRows.length > 0) {
        ...
        const totalPackingPlans = (totalPackingPlans || 0) + packingPlanResult.affectedRows;
        ...
        // 4.4-4.14 级联删阄11个新工序计划
        const [shotBlastingPlanResult] = await connection.execute(
          'DELETE FROM shot_blasting_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalShotBlastingPlans = (totalShotBlastingPlans || 0) + shotBlastingPlanResult.affectedRows;
        
        const [manualWeldingPlanResult] = await connection.execute(
          'DELETE FROM manual_welding_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalManualWeldingPlans = (totalManualWeldingPlans || 0) + manualWeldingPlanResult.affectedRows;
        
        const [tubeBendingPlanResult] = await connection.execute(
          'DELETE FROM tube_bending_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalTubeBendingPlans = (totalTubeBendingPlans || 0) + tubeBendingPlanResult.affectedRows;
        
        const [laserTubeCuttingPlanResult] = await connection.execute(
          'DELETE FROM laser_tube_cutting_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalLaserTubeCuttingPlans = (totalLaserTubeCuttingPlans || 0) + laserTubeCuttingPlanResult.affectedRows;
        
        const [laserCuttingPlanResult] = await connection.execute(
          'DELETE FROM laser_cutting_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalLaserCuttingPlans = (totalLaserCuttingPlans || 0) + laserCuttingPlanResult.affectedRows;
        
        const [bendingPlanResult] = await connection.execute(
          'DELETE FROM bending_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalBendingPlans = (totalBendingPlans || 0) + bendingPlanResult.affectedRows;
        
        const [drillingPlanResult] = await connection.execute(
          'DELETE FROM drilling_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalDrillingPlans = (totalDrillingPlans || 0) + drillingPlanResult.affectedRows;
        
        const [punchingPlanResult] = await connection.execute(
          'DELETE FROM punching_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalPunchingPlans = (totalPunchingPlans || 0) + punchingPlanResult.affectedRows;
        
        const [manualCuttingPlanResult] = await connection.execute(
          'DELETE FROM manual_cutting_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalManualCuttingPlans = (totalManualCuttingPlans || 0) + manualCuttingPlanResult.affectedRows;
        
        const [machineGrindingPlanResult] = await connection.execute(
          'DELETE FROM machine_grinding_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalMachineGrindingPlans = (totalMachineGrindingPlans || 0) + machineGrindingPlanResult.affectedRows;
        
        const [cuttingPlanResult] = await connection.execute(
          'DELETE FROM cutting_process_plans WHERE sales_order_no = ?',
          [internalOrderNo]
        );
        const totalCuttingPlans = (totalCuttingPlans || 0) + cuttingPlanResult.affectedRows;
